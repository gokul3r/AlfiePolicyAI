...
- **`VoiceChatDialog`**: Voice-based AI assistant with WebSocket audio streaming and real-time transcription using Google Gemini Live API (16kHz input, 24kHz output). **AI Agent Architecture**: Uses Gemini's function calling (tool use) instead of keyword matching. Annie (the AI) decides when to call functions based on understanding context and intent. **Available Tools**:
  ...
  **Flow**: User asks for quotes → Annie calls `get_user_vehicles` → Shows vehicle details → User confirms → Annie calls `search_quotes` → Quotes displayed → User says "go with Baviva" → Annie calls `select_quote("Baviva")` → Backend fuzzy-matches to actual quote → Annie confirms → User says "yes" → Annie calls `show_payment` → Payment card shown → User says "confirm payment" → Annie calls `complete_purchase` → Database updated. **Two-Stage Purchase Flow**: Requires explicit payment confirmation ("confirm payment", "pay now", "complete purchase") before calling `complete_purchase`.
