...
-   **Provider Customer Agent App** (`client/src/pages/CustomerAgentPage.tsx`): A separate app at `/customer_agent` designed to be opened on a different browser/device by the insurance provider's customer support agent. Features: (1) Same password gate as AutoAnnie (AA@ITCTO), (2) Email-based login where provider is extracted from email domain (e.g., `customer_agent@admiral.com` → "Admiral Insurance Customer Support"), (3) Dynamic branding with provider-specific color schemes, (4) Notification bell with badge count for pending retention requests, (5) Negotiation cards showing customer details, policy info, and competitor quotes, (6) Match / Partially Match / Unable to Match action buttons with price input. Uses polling (3s interval) to fetch new negotiations. Communicates with AutoAnnie via the `negotiations` database table and API endpoints.
