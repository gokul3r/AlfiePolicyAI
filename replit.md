...
- **`ChatDialog`**: Text-based AI assistant chat interface with message history persistence and real-time updates using OpenAI Responses API. **Quote Search via Chat**: When user sends messages like "search for quotes", "find insurance", "buy a policy", etc., the chat detects the intent and automatically searches for quotes using the user's first vehicle policy. Returns top 3 quotes sorted by Auto Annie Score (alfie_touch_score) showing insurer name, score, and AI message. If user has no vehicle policy, responds with guidance to add a policy first.
