...
- **`ScheduleQuoteDialog`**: Two-column responsive dialog (stacks on mobile) for scheduling automated quote searches. Features vehicle selector dropdown (left), monthly/weekly frequency toggle (right), and calculated next search date preview. Includes magical "Timelapse" button with shimmer animation and sparkles icon to showcase scheduled quote search functionality. State management resets on open/close and syncs with parent props. Currently stores preferences locally (backend integration pending).
- **`TimelapseDialog`**: Showcase dialog for Auto-Annie's scheduled quote search feature. Opens from ScheduleQuoteDialog's magical Timelapse button. Features animated intro text ("Experience how Auto-Annie's scheduled quote search works"), accessible close button using DialogClose primitive, and placeholder "Start" button for future demo functionality. Uses shimmer gradient animation (purple-to-blue) and entrance animations for engaging UX.
...
- **`QuoteCard`**: Displays individual quotes with insurer info, Trustpilot rating, features matching, AutoAnnie Score, and AI analysis.
- **`ChatDialog`**: Text-based AI assistant chat interface with message history persistence and real-time updates using OpenAI Responses API. Supports conversational add-policy flow where users can: (1) request to add a car via chat, (2) upload PDF policy documents for automatic 15-field extraction, (3) confirm extracted details, (4) search for quotes, and (5) purchase insurance - all through natural conversation. Includes QuoteChatCard display for expandable quote cards in chat, file upload integration, purchase progress animations, and "Load More" navigation to full QuotesScreen. AI uses action markers ([ACTION:SHOW_UPLOAD], [ACTION:SAVE_POLICY], [ACTION:SEARCH_QUOTES], [ACTION:PURCHASE_POLICY:provider]) to trigger UI elements.
- **`QuoteChatCard`**: Expandable quote card component for chat interface. Displays insurer name, logo, annual price, AutoAnnie score, and key features. Expands on click to show AutoAnnie's AI analysis, included features (green checks), and missing features (red x). Features smooth collapse/expand animations and mobile-friendly touch targets.
